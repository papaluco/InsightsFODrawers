...
import { ENGAGEMENT_TIERS, EngagementTierKey, calcInsightsUserScore, getEngagementTier } from '../../../utils/engagementTiers';
...
const InsightsUserEngagementChart: React.FC<Props> = ({ data }) => {
  ...
  const chartData = useMemo(() => {
    ...
    const allScores = data.map(calcInsightsUserScore);
    ...
    return TIER_ORDER.map(key => ({
      key,
      label: ENGAGEMENT_TIERS[key].label,
      count: counts[key],
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      >
        <div className="text-left">
          <span className="text-sm font-semibold text-slate-700">User Engagement Distribution</span>
          <span className="ml-2 text-[11px] text-gray-400">Based on session + interaction depth score</span>
        </div>
                    ...
                    content={({ active, payload }) => {
                      ...
                      return (
                        <div className="bg-white shadow-lg border border-gray-100 rounded-xl px-3 py-2 text-xs">
                          ...
                          <p className="text-gray-500">{d.count.toLocaleString()} user{d.count !== 1 ? 's' : ''}</p>
              ...
              <div className="flex flex-wrap gap-3 mt-2">
                {chartData.map(d => (
                  ...
                  >
                    ...
                    <span className="text-gray-500 font-medium">{d.count} user{d.count !== 1 ? 's' : ''}</span>
...
export default React.memo(InsightsUserEngagementChart);
