...
const InsightsDistrictActivityTrend: React.FC<Props> = ({ data, events }) => {
  ...
  const districtTzMap = useMemo(() => {
    ...
    data.forEach(d => { map[d.districtId] = d.timezone; });
    return map;
  }, [data]);

  const chartData = useMemo(() => {
    const districtIdSet = new Set(data.map(d => d.districtId));
    if (districtIdSet.size === 0) return [];

    const bucketMap = new Map<string, { morning: number; afternoon: number; evening: number }>();

    events
      .filter(e => districtIdSet.has(e.districtId))
      .forEach(e => {
        const bucket = getBucket(e.timestamp, granularity);
        const tz = timeMode === 'district' ? districtTzMap[e.districtId] : undefined;
  ...
  }, [data, events, granularity, timeMode, districtTzMap]);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3.5">
        ...
        >
          <span className="text-sm font-semibold text-slate-700">District Activity Trend</span>
...
export default React.memo(InsightsDistrictActivityTrend);
