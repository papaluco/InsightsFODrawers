...
type CountMode = 'pageViews' | 'sessions';
...
const InsightsOverviewActivityTrend: React.FC<Props> = ({ events, districtTimezones }) => {
  const [expanded, setExpanded] = useState(true);
  const [granularity, setGranularity] = useState<Granularity>('week');
  const [timeMode, setTimeMode] = useState<TimeMode>('district');
  const [countMode, setCountMode] = useState<CountMode>('pageViews');

  const chartData = useMemo(() => {
    const bucketMap = new Map<string, { morning: number; afternoon: number; evening: number }>();

    // For sessions mode: pick the earliest event per session to represent it
    let sourceEvents: InsightsUsageEvent[];
    if (countMode === 'pageViews') {
      sourceEvents = events.filter(e => e.eventType === 'INSIGHTS_PAGE_VIEWED');
    } else {
      // One representative event per session (first by timestamp)
      const sessionFirst = new Map<string, InsightsUsageEvent>();
      events.forEach(e => {
        const existing = sessionFirst.get(e.sessionId);
        if (!existing || e.timestamp < existing.timestamp) sessionFirst.set(e.sessionId, e);
      });
      sourceEvents = [...sessionFirst.values()];
    }
  ...
  }, [events, granularity, timeMode, countMode, districtTimezones]);
        ...
        <div className="flex items-center gap-2">
          <div className="flex rounded-lg border border-gray-200 overflow-hidden text-[11px] font-semibold">
            <button
              ...
              className={`px-2.5 py-1 transition-colors ${countMode === 'pageViews' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50'}`}
            >
              Page Views
            </button>
            <button
              onClick={() => setCountMode('sessions')}
              className={`px-2.5 py-1 transition-colors ${countMode === 'sessions' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50'}`}
            >
              Sessions
            </button>
          </div>
...
export default React.memo(InsightsOverviewActivityTrend);
