...
import { ChevronUp, ChevronDown, MessageSquare, Star, GripVertical } from 'lucide-react';
import { InsightsUserStatRow } from '../../../types/insightsUsageTypes';
...
import { getEngagementTier, calcInsightsUserScore, EngagementTier } from '../../../utils/engagementTiers';
...
type ScoredRow = InsightsUserStatRow & { _score: number; _tier: EngagementTier };

type ColKey = 'userName' | 'districtName' | 'platform' | 'sessions' | 'pageViews' |
  'interactions' | 'drawerOpens' | 'schoolieUsage' | 'downloads' | 'avgSessionsPerWeek' |
  'lastActive' | 'engagement';
...
const COLUMNS: ColDef[] = [
  {
    ...
    sortValue: r => r.userName, getValue: r => r.userName,
    render: r => <span className="text-sm font-medium text-slate-700">{r.userName}</span>,
  },
  {
    key: 'districtName', label: 'District', defaultVisible: true,
    sortValue: r => r.districtName, getValue: r => r.districtName,
    render: r => <span className="text-sm text-slate-500">{r.districtName}</span>,
  },
  {
  ...
  {
    key: 'avgSessionsPerWeek', label: 'Avg Sess/Wk', defaultVisible: false,
    sortValue: r => r.avgSessionsPerWeek, getValue: r => r.avgSessionsPerWeek.toFixed(2),
    render: r => <span className="text-sm text-slate-500 tabular-nums">{r.avgSessionsPerWeek.toFixed(2)}</span>,
  },
  {
    key: 'lastActive', label: 'Last Active', defaultVisible: true,
    sortValue: r => r.lastActive, getValue: r => fmtDate(r.lastActive),
    render: r => <span className="text-sm text-slate-400 whitespace-nowrap">{fmtDate(r.lastActive)}</span>,
  },
  {
...
interface Props {
  ...
  onRowClick?: (row: InsightsUserStatRow) => void;
}

const InsightsUserGrid: React.FC<Props> = ({ data, onRowClick }) => {
  ...
  const scored = useMemo<ScoredRow[]>(() => {
    const allScores = data.map(r => calcInsightsUserScore(r));
  ...
  const csvData = useMemo((): ICSVReportData => ({
    fileName: `Insights_Users_${new Date().toISOString().slice(0, 10)}`,
    ...
    <div className="rounded-xl border border-gray-200 shadow-sm bg-white">
      ...
      <div className="flex items-center justify-between px-5 py-3.5">
        <button onClick={() => setExpanded(e => !e)} className="flex items-center gap-2.5 hover:opacity-80 transition-opacity">
          <span className="text-sm font-semibold text-slate-700">Users Detail</span>
                ...
                {paged.map(row => (
                  ...
                  >
                    <td className="px-4 py-2.5">
                      {row.isPowerUser && (
                        <span title="Power User"><Star size={13} className="text-amber-400 fill-amber-400" /></span>
                      )}
                    </td>
                    {orderedVisibleCols.map(col => (
...
export default React.memo(InsightsUserGrid);
