...
type SortKey = 'userName' | 'districtName' | 'helpful' | 'notHelpful' | 'total' | 'pctHelpful';
...
const FeedbackByUserTable: React.FC<Props> = ({ data, onUserClick }) => {
  ...
  const rows = useMemo(() => {
    const map = new Map<string, { helpful: number; notHelpful: number; districtId: string }>();
    data.forEach(r => {
      const cur = map.get(r.userId) ?? { helpful: 0, notHelpful: 0, districtId: r.districtId };
      if (r.feedbackValue === 'thumbs_up') cur.helpful++;
      else cur.notHelpful++;
      map.set(r.userId, cur);
    });
    return [...map.entries()].map(([userId, c]) => ({
      userId,
      userName: USER_DISPLAY_NAMES[userId] ?? userId,
      districtName: DISTRICT_DISPLAY_NAMES[c.districtId] ?? c.districtId,
      helpful: c.helpful,
      notHelpful: c.notHelpful,
      total: c.helpful + c.notHelpful,
      pctHelpfulNum: (c.helpful + c.notHelpful) > 0 ? c.helpful / (c.helpful + c.notHelpful) : 0,
    }));
  }, [data]);
  ...
  const sorted = useMemo(() => [...rows].sort((a, b) => {
    const val = (r: typeof rows[0]): string | number =>
      ...
      sortConfig.key === 'userName'   ? r.userName :
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
      <div className="flex w-full items-center justify-between px-5 py-4 hover:bg-gray-50 rounded-xl">
        ...
        >
          <h4 className="text-sm font-semibold text-gray-900">Feedback by User</h4>
        </button>
        <button
          type="button"
          onClick={() => setExpanded(prev => !prev)}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gray-50">
                      <Th label="User" sortKey="userName" />
                  ...
                  <tbody>
                    {paged.map(row => (
                      <tr
                        key={row.userId}
                        onClick={() => onUserClick(row.userId, row.userName)}
                      ...
                      >
                        ...
                        <td className="py-2.5 px-3 text-gray-500">{row.districtName}</td>
...
export default FeedbackByUserTable;
