...
import { PIE_COLORS, pct } from './feedbackHelpers';
...
const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: { payload: PieDataItem }[] }) => {
  ...
  const pctH = d.value > 0 ? Math.round((d.helpful / d.value) * 100) : 0;
  return (
    <div className="bg-white border border-gray-200 shadow-lg rounded-lg p-3 text-sm">
      ...
      <p className="text-gray-500">Count: {d.value}</p>
      <p className="text-green-600">👍 {d.helpful} ({pctH}% helpful)</p>
      <p className="text-red-500">👎 {d.notHelpful}</p>
    </div>
  );
};

const FeedbackPieChart: React.FC<Props> = ({
  data,
  title,
  colors = PIE_COLORS,
  onSegmentClick,
              ...
              <div className="flex-1 min-w-0 space-y-1">
                {data.map((item, i) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 py-1.5 px-2 rounded-lg transition-colors text-left ${onSegmentClick ? 'hover:bg-gray-50 cursor-pointer' : ''}`}
                  >
                    <div
                    ...
                    <span className="text-sm font-semibold text-gray-800 tabular-nums text-right w-8 flex-shrink-0">{item.value}</span>
                    <span className="text-xs text-gray-400 text-right w-12 flex-shrink-0">{item.pctLabel ?? pct(item.helpful, item.value)}</span>
                  </button>
                ))}
              </div>
            </div>
