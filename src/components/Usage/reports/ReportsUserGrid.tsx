...
import { UserStatRow } from '../../../types/reportUsageTypes';
...
interface Props {
  data: UserStatRow[];
  onRowClick?: (row: UserStatRow) => void;
}

type SortKey = keyof UserStatRow;
...
const ReportsUserGrid: React.FC<Props> = ({ data, onRowClick }) => {
  ...
  const filtered = useMemo(() => {
    let rows = search
      ? data.filter(r => r.userName.toLowerCase().includes(search.toLowerCase()) || r.districtName.toLowerCase().includes(search.toLowerCase()))
      : data;
    rows = [...rows].sort((a, b) => {
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100">
                {paged.map(row => (
                  ...
                  >
                    <td className="px-4 py-2.5 text-sm font-medium text-slate-700">{row.userName}</td>
                    <td className="px-4 py-2.5 text-sm text-slate-500">{row.districtName}</td>
                    ...
                    <td className="px-4 py-2.5 text-sm text-slate-400 whitespace-nowrap">{fmtDate(row.lastActive)}</td>
...
export default ReportsUserGrid;
