...
import { getPromptName, getPromptTypeDisplay, getReasonDisplay } from '../feedback/feedbackHelpers';
...
const SchoolieFeedbackDetail: React.FC<Props> = ({ record, isOpen, onClose }) => {
  ...
  const userName = record ? (SCHOOLIE_USER_NAMES[record.userId] ?? record.userId) : '';
  const districtName = record
    ? ((record.contextJson?.districtName as string) ?? SCHOOLIE_DISTRICT_NAMES[record.districtId] ?? record.districtId)
    : '';
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div
        className={`fixed inset-y-0 right-0 w-1/4 min-w-80 bg-white z-[60] shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
...
export default SchoolieFeedbackDetail;
