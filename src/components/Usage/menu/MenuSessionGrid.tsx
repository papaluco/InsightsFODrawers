...
import { MenuSessionRow } from '../../../types/menuUsageTypes';
...
interface Props {
  data: MenuSessionRow[];
  onRowClick?: (row: MenuSessionRow) => void;
  onEventsClick?: (row: MenuSessionRow) => void;
  onUserClick?: (row: MenuSessionRow) => void;
  onDistrictClick?: (row: MenuSessionRow) => void;
}

type ColKey =
  | 'userName' | 'districtName' | 'platform'
  | 'eventCount' | 'pageViews' | 'interactions'
  | 'menuItemsDrawerViews' | 'schoolPerformanceDrawerViews'
  | 'firstEvent' | 'lastEvent' | 'sessionId';
...
const COLUMNS: ColDef[] = [
  ...
  { key: 'menuItemsDrawerViews',         label: 'Menu Items',   defaultVisible: true, sortable: true },
  { key: 'schoolPerformanceDrawerViews', label: 'School Perf.', defaultVisible: true, sortable: true },
  { key: 'firstEvent',                   label: 'Start',        defaultVisible: true, sortable: true },
  { key: 'lastEvent',                    label: 'Last Event',   defaultVisible: true, sortable: true },
...
function getSortValue(row: MenuSessionRow, key: ColKey): string | number {
  switch (key) {
    case 'userName':                     return MENU_USER_NAMES[row.userId] ?? row.userId;
    case 'districtName':                 return MENU_DISTRICT_NAMES[row.districtId] ?? row.districtId;
    case 'platform':                     return row.platform;
    case 'sessionId':                    return row.sessionId;
    case 'eventCount':                   return row.eventCount;
    case 'pageViews':                    return row.pageViews;
    case 'interactions':                 return row.interactions;
    case 'menuItemsDrawerViews':         return row.menuItemsDrawerViews;
    case 'schoolPerformanceDrawerViews': return row.schoolPerformanceDrawerViews;
    case 'firstEvent':                   return row.firstEvent;
    case 'lastEvent':                    return row.lastEvent;
    default:                             return '';
  }
}

function getCsvValue(row: MenuSessionRow, key: ColKey): string {
  switch (key) {
    case 'userName':                     return MENU_USER_NAMES[row.userId] ?? row.userId;
    case 'districtName':                 return MENU_DISTRICT_NAMES[row.districtId] ?? row.districtId;
    case 'platform':                     return row.platform;
    ...
    case 'firstEvent':                   return fmtDateTime(row.firstEvent);
    case 'lastEvent':                    return fmtDateTime(row.lastEvent);
    default:                             return '';
  }
}
...
const MenuSessionGrid: React.FC<Props> = ({ data, onRowClick, onEventsClick, onUserClick, onDistrictClick }) => {
  const [expanded, setExpanded] = useState(true);
  const [search, setSearch] = useState('');
  const [sort, setSort] = useState<{ key: ColKey; dir: 'asc' | 'desc' }>({ key: 'firstEvent', dir: 'desc' });
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      rows = rows.filter(r =>
        (MENU_USER_NAMES[r.userId] ?? r.userId).toLowerCase().includes(q) ||
        (MENU_DISTRICT_NAMES[r.districtId] ?? r.districtId).toLowerCase().includes(q) ||
  ...
  const csvData = useMemo((): ICSVReportData => ({
    fileName: `menu_sessions_${new Date().toISOString().slice(0, 10)}`,
                  ...
                  <tr key={row.sessionId} className="transition-colors hover:bg-slate-50">
                    {orderedVisibleCols.map(col => {
                      ...
                      return (
                        ...
                        >
                          {col.key === 'userName'    && <span className="font-medium text-slate-700">{MENU_USER_NAMES[row.userId] ?? row.userId}</span>}
                          {col.key === 'districtName' && (MENU_DISTRICT_NAMES[row.districtId] ?? row.districtId)}
                          {col.key === 'sessionId'   && <span className="font-mono text-[11px] text-slate-400">{row.sessionId.slice(0, 8)}…</span>}
                          {col.key === 'platform'    && row.platform}
                          {col.key === 'eventCount'  && <span className="tabular-nums font-semibold">{row.eventCount}</span>}
                          ...
                          {col.key === 'firstEvent'  && <span className="text-slate-400">{fmtDateTime(row.firstEvent)}</span>}
                          {col.key === 'lastEvent'   && <span className="text-slate-400">{fmtDateTime(row.lastEvent)}</span>}
                        </td>
                      );
                    })}
                  </tr>
...
export default MenuSessionGrid;
