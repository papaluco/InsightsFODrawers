...
const COLUMNS: ColDef[] = [
  ...
  { key: 'filterChanges',               label: 'Filter Changes',     defaultVisible: false, sortable: true  },
...
function getCsvValue(row: MenuDistrictStatRow, key: ColKey): string {
  switch (key) {
...
const MenuDistrictsGrid: React.FC<Props> = ({
  ...
  const csvData = useMemo((): ICSVReportData => ({
    fileName: `menu_districts_${new Date().toISOString().slice(0, 10)}`,
                  ...
                  {orderedVisibleCols.map(col => (
                    ...
                    >
                      <div className="flex items-center gap-1">
                        {col.label}
                        {col.sortable && sort.key === (col.key as SortKey) && (
                          sort.dir === 'asc'
...
export default MenuDistrictsGrid;
